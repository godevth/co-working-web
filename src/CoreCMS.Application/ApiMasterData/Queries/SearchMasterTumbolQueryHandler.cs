...
{
    public class SearchMasterTumbolQueryHandler : IRequestHandler<SearchMasterTumbolQuery, CommandResult<List<MasterTumbol>>>
    {
        private readonly ApiConfig _config;
        private readonly IMediator _mediator;

        public SearchMasterTumbolQueryHandler(IOptions<ApiConfig> config, IMediator mediator)
        {
            _config = config.Value;
            _mediator = mediator;
        }

        public async Task<CommandResult<List<MasterTumbol>>> Handle(SearchMasterTumbolQuery request, CancellationToken cancellationToken)
        {
            CommandResult<List<MasterTumbol>> cmdResult = new CommandResult<List<MasterTumbol>>()
            {
                Message  = "ไม่สามารถเรียก API Tumbol ได้"
            };
            Response<List<MasterTumbol>> response = null;
            ...
            req.AddJsonBody(
                new{
                    P_AMPHER_ID = !string.IsNullOrEmpty(request.AmpherId) ? request.AmpherId : "",
                    P_TUMBOL_ID = !string.IsNullOrEmpty(request.TumbolId) ? request.TumbolId : ""
                }
            );
            ...
            if (result.StatusCode == System.Net.HttpStatusCode.OK)
            {
                ...
                {
                    response = JToken.Parse(result.Content).ToObject<Response<List<MasterTumbol>>>();
                }

                cmdResult.Message = "เรียก API Tumbol สำเร็จ";

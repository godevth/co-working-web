              ...
              <div class="row">
                <div class="col-9">
                  <v-text-field
                    ...
                    :hint="$t('ROLE.SEARCH.KEYWORD_HINT')"
...
export default {
  ...
  data() {
    return {
      ...
      datatable: {
        headers: [
          {
            text: this.$t("ROLE.SEARCH.NAME"),
            value: "name",
            align: "center",
          },
          {
            text: this.$t("ROLE.SEARCH.DESCRIPTION"),
            value: "description",
            align: "center",
          },
                    {
            text: this.$t("ROLE.SEARCH.USER_TYPE"),
            value: "userTypeName",
            align: "center",
          },
          // {
          //   text: this.$t("ROLE.SEARCH.IN_ACTIVE_STATUS"),
          //   value: "inActiveStatus",
          //   align: "center",
          // },
          {
            text: this.$t("ROLE.SEARCH.ACTION"),
            value: "id",
            align: "center",
            sortable: false,
            width: "10%"
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      return new Promise((resolve) => {
        ApiService.setHeader();
        ApiService.post("/Api/Role/Search", {
    ...
    editItem(id){
      this.$router.push({ name: "editRole", params: { id } });
    },
    ...
    confirmDeleteItem(){
      ...
      ApiService.setHeader();
      ApiService.update("/Api/Role", this.datatable.deleteDialogId +"/Delete")
        .then(({ data }) => {
          //console.log(data);
          this.datatable.dialog = true;
          if (data.message) {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [
      ...
      { title: this.$t("MENU.ROLE.SEARCH") },
    ]);
  },
  computed: {
    title() {
      return this.$t("MENU.ROLE.SEARCH");
    },
  },
  watch: {
    "datatable.options": {

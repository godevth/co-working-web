...
export default {
  ...
  data() {
    return {
      ...
      datatable: {
        headers: [
          {
            text: this.$t("FACILITY.SEARCH.NAME_TH"),
            value: "name_TH",
            align: "center",
          },
          {
            text: this.$t("FACILITY.SEARCH.NAME_EN"),
            value: "name_EN",
            align: "center",
          },
                    {
            text: this.$t("FACILITY.SEARCH.FACILITY_TYPE"),
            value: "facilityTypeName",
            align: "center",
          },
          {
            text: this.$t("FACILITY.SEARCH.IN_ACTIVE_STATUS"),
            value: "inActiveStatus",
            align: "center",
          },
          {
            text: this.$t("FACILITY.SEARCH.ACTION"),
            value: "id",
            align: "center",
            sortable: false,
            width: "10%"
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      return new Promise((resolve) => {
        ApiService.setHeader();
        ApiService.post("/Api/Facility/Search", {
          FacilityTypeId:this.form.facilityType
            ? this.form.facilityType.value
            : null,
    ...
    editItem(id){
      this.$router.push({ name: "editFacility", params: { id } });
    },
    ...
    confirmDeleteItem(){
      ...
      ApiService.setHeader();
      ApiService.update("/Api/Facility", this.datatable.deleteDialogId +"/Delete")
        .then(({ data }) => {
          //console.log(data);
          this.datatable.dialog = true;
          if (data.message) {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [
      ...
      { title: this.$t("FACILITY._FACILITY.SEARCH") },
    ]);
  },
  computed: {
    title() {

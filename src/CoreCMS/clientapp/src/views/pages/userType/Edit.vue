              ...
              <v-row>
                ...
                >
                  <v-text-field
                    ...
                    :counter="256"
                    :label="$t('USER_TYPE.ADD_EDIT.NAME')"
                ...
                >
                  <v-textarea
                    ...
                    :disabled="form.loading"
                    :counter="450"
                    :label="$t('USER_TYPE.ADD_EDIT.DESCRIPTION')"
                ...
                <v-col cols="12" md="12">
                  <v-checkbox v-model="form.inActiveStatus"
                    :disabled="form.loading"
                    :label="$t('USER_TYPE.SEARCH.IN_ACTIVE_STATUS')"
                    ></v-checkbox>
                </v-col>
...
export default {
  ...
  data() {
    return {
      ...
      form: {
        ...
        loading: false,
        nameRules: [
          (v) => !!v || this.$t("USER_TYPE.ADD_EDIT.NAME_VALIDATION"),
            (v) => (v && v.length <= 450) || this.$t("USER_TYPE.ADD_EDIT.NAME_COUNTER"),
            (v) => (/[a-z_]+$/.test(v)) || this.$t("USER_TYPE.ADD_EDIT.NAME_FORMAT")
        ],
        descriptionRules: [
            (v) => !!v || this.$t("USER_TYPE.ADD_EDIT.DESCRIPTION_VALIDATION"),
            (v) => (v && v.length <= 450) || this.$t("USER_TYPE.ADD_EDIT.DESCRIPTION_COUNTER"),
        ],
        errors: [],
        name: "",
        nameEN: "",
  ...
  computed: {
    title() {
          return this.$t("MENU.USER_TYPE.EDIT");
    },
  },
  methods: {
    submitForm() {
    ...
    postDataToApi() {
      ...
      ApiService.put("/Api/UserType/Edit", {
        Id: this.form.id,
        Name: this.form.name,
    ...
    closeDialog() {
      ...
      this.$router.push({ name: "searchUserType" });
    },
    getDataFromApi(id) {
      //var self = this;
      this.form.loading = true;
      return new Promise(() => {
        ApiService.setHeader();
          ApiService.get("/Api/UserType", id)
          .then(({ data }) => {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [
      { title: this.$t("MENU.USER_TYPE.SECTION"), route: "/userType" },
        { title: this.$t("MENU.USER_TYPE.EDIT") }
    ]);

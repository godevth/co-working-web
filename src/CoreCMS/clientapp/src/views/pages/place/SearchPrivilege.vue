          ...
          <template v-slot:body>
            <v-col cols="12" md="12">
              <v-text-field
                ...
                :disabled="true"
                :label="$t('PRIVILEGE.SEARCH.PLACE_NAME')"
                required
              ></v-text-field>
            </v-col>
              ...
              <div class="row">
                <div class="col-4">
                  <v-text-field
                    ...
                    :hint="$t('PRIVILEGE.SEARCH.KEYWORD_HINT')"
                    prepend-icon="mdi-file-document-box-search-outline"
                  ></v-text-field>
                </div>
                <div class="col-4">
                  <v-autocomplete
                    v-model="form.privilegeType"
                    :disabled="datatable.loading"
                    :items="form.privilegeTypeItems"
                    :loading="form.privilegeTypeLoading"
                    :search-input.sync="form.privilegeTypeSearch"
                    hide-no-data
                    hide-selected
                    item-text="text"
                    item-value="value"
                    :label="$t('PRIVILEGE.SEARCH.PRIVILEGE_TYPE_NAME')"
                ...
                <div class="col-4">
                  <v-autocomplete
                    ...
                    item-text="text"
                    item-value="value"
                    :label="$t('PRIVILEGE.SEARCH.IN_ACTIVE_STATUS')"
                    :placeholder="$t('SHARED.START_TYPING_TO_SEARCH')"
                    prepend-icon="mdi-database-search"
                    return-object
                    clearable
            ...
            <v-subheader>
              ...
              <v-btn
                ...
                type="button"
                tile
                :to="'/place/addCustumer/' + form.placeId"
                target="_self"
              >
                <v-icon left>add_circle_outline</v-icon>
                {{ $t("PRIVILEGE._PRIVILEGE.ADD_CUSTUMER") }}
              </v-btn>
              ...
              <v-btn
                ...
                type="button"
                tile
                :to="'/place/addPrivilege/' + form.placeId"
                target="_self"
              >
                <v-icon left>add_circle_outline</v-icon>
                {{ $t("PRIVILEGE._PRIVILEGE.ADD") }}
              </v-btn>
              ...
              <v-btn
                ...
                type="button"
                tile
                :to="'/place/importCustumer/' + form.placeId"
                target="_self"
              >
                <v-icon left> mdi-upload</v-icon>
                {{ $t("PRIVILEGE._PRIVILEGE.IMPORT_CUSTUMER") }}
              </v-btn>
            </v-subheader>
            ...
            >
              <template v-slot:item.inActiveStatus="{ item }">
                <v-btn
                  ...
                  class="mr-4"
                  tile
                  @click="toggleItem(item.privilegeId, item.inActiveStatus)"
                >
                  {{ getInActiveStatusText(item.inActiveStatus) }}
                </v-btn>
              </template>

              <template v-slot:item.privilegeId="{ item }">
                <v-icon medium @click="deleteItem(item.privilegeId)">
                  mdi-delete
                </v-icon>
              </template>
            </v-data-table>
...
export default {
  ...
  data() {
    return {
      ...
      datatable: {
        headers: [
          {
            text: this.$t("PRIVILEGE.SEARCH.PRIVILEGE_TYPE_NAME"),
            value: "privilegeTypeName",
            align: "center",
          },
          {
            text: this.$t("PRIVILEGE.SEARCH.DOMAIN"),
            value: "domain",
            align: "center",
          },
          {
            text: this.$t("PRIVILEGE.SEARCH.EMAIL"),
            value: "email",
            align: "center",
          },
          {
            text: this.$t("PRIVILEGE.SEARCH.DISPLAY_NAME"),
            value: "displayName",
            align: "center",
          },
          {
            text: this.$t("PRIVILEGE.SEARCH.PHONE_NUMBER"),
            value: "phoneNumber",
            align: "center",
          },
          {
            text: this.$t("PRIVILEGE.SEARCH.IN_ACTIVE_STATUS"),
            value: "inActiveStatus",
            align: "center",
          },
          {
            text: this.$t("PRIVILEGE.SEARCH.ACTION"),
            value: "privilegeId",
            align: "center",
            sortable: false,
            width: "10%",
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      return new Promise((resolve) => {
        ...
        ApiService.post("/Api/Privilege/Search", {
          ...
          PlaceId: this.$route.params.id,
          PrivilegeTypeCode: this.form.privilegeType
            ? this.form.privilegeType.value
            : null,
    ...
    getPrivilegeTypeFromApi() {
      return new Promise((resolve) => {
        ApiService.setHeader();
        ApiService.post("/Api/Variables/Options", {
          Search: "PRIVILEGE_TYPE",
        })
          .then(({ data }) => {
            resolve(data);
          })
          .finally(() => {
            this.form.privilegeTypeLoading = false;
          });
      });
    },
    getPlaceApi(id) {
  ...
  mounted() {
    this.$store.dispatch(SET_BREADCRUMB, [
      ...
      { title: this.$t("PRIVILEGE._PRIVILEGE.SEARCH") },
    ]);
  },
  computed: {
    title() {
      return this.$t("PRIVILEGE._PRIVILEGE.SEARCH");
    },
  },
  watch: {
    "datatable.options": {
    ...
    "form.privilegeTypeSearch": {
      handler() {
        this.getPrivilegeTypeFromApi().then((data) => {
          this.form.privilegeTypeItems = data;
        });
      },
    },
  },

                    ...
                    <template v-slot:body>
                        <v-form ref="form"
                            ...
                            <v-row>
                                ...
                                <!-- <v-flex d-flex>
                                    <v-layout wrap>
                                        <v-flex md4 v-for="item in form.pictureFile" :key="item.id">
                                            <v-card class="card-container">
                                                {{ item.url }}<v-btn @click="deleteItem(item.id)">Delete</v-btn>
                                            </v-card>
                                        </v-flex>
                                    </v-layout>
                                </v-flex> -->
                                <v-col cols="12" md="6" v-for="item in pictureFile" :key="item.id">
                                    <v-img :v-model="pictureFile"
                                           v-if="item.id"
                                           :src="item.url"
                                           :disabled="form.loading"
                                           aspect-ratio="1.7"
                                           contain>
                                        <v-icon medium
                                                style="position:absolute;top:10px;right:21%;"
                                                class="mr-2 btn m-btn--pill btn-danger"
    ...
    export default {
        ...
        methods: {
            ...
            getDataFromApi(id) {
                ...
                return new Promise(() => {
                    ApiService.setHeader();
                    ApiService.get("/Api/Room", id)
                        .then(({ data }) => {
                            this.form.id = data.data.id;
                            this.form.nameTH = data.data.name_TH;
                            this.form.nameEN = data.data.name_EN;
                            this.form.qty = data.data.qty;
                            this.form.capacity = data.data.capacity;
                            this.form.price = data.data.price;
                            this.form.inActiveStatus = !data.data.inActiveStatus;
                            this.form.placeId = data.data.placeId;
                            this.form.detailsTH = data.data.detail_TH;
                            this.form.detailsEN = data.data.detail_EN;
                            this.form.roomType = { value: data.data.roomTypeId };
                            ...
                            if (data.data.price.length > 0) {
                                ...
                                for (let i = 0; i < data.data.price.length; i++) {
                                    var pitem = data.data.price[i];
                                    var pri = {
                                        value: pitem.timeType
                                    }
                                    this.priceItems.push({
                                        id: pitem.id,
                                        timeType: pri,
                                    ...
                                    setTimeout(function () {
                                        self.$refs.priItems[i].timeTypeItem = {
                                            value: data.data.price[i].timeType
                                        };
                                    }, 300);
                                }
                            }
                            ...
                            if (data.data.pictures.length > 0) {
                                for (let i = 0; i < data.data.pictures.length; i++) {
                                    ...
                                    setTimeout(function () {
                                        self.pictureFile.push({
                                            Base64s: null,
                                            url: data.data.pictures[i].downloadFileByteUrl,
                                            base64Url: data.data.pictures[i].downloadFileBase64Url,
                                            id: data.data.pictures[i].id,
                                            ContentTypes: null
                                        });
                                    }, 100);
                                }

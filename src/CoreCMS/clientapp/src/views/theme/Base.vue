    ...
    <v-dialog v-model="showLoginDialog" persistent max-width="500">
      <v-card>
        <v-card-title class="headline">{{
          $t("AUTH.UNAUTHENTICATED_MODAL.TITLE")
        }}</v-card-title>
        <v-card-text>
          <v-icon color="red" x-large>fa fa-exclamation-triangle</v-icon>
          {{ $t("AUTH.UNAUTHENTICATED_MODAL.TEXT") }}
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" text @click="goLogin">
            {{ $t("AUTH.UNAUTHENTICATED_MODAL.LOGIN_BUTTON") }}
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
